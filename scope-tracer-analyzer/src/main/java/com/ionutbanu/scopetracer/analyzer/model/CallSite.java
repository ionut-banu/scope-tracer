...
/**
 * The source location of a {@code fork()} call, captured independently of the task's display label
 * ({@link TaskRecord#taskName()}) — populated even when the label was supplied explicitly.
 *
 * @param className simple name of the declaring class (the lambda's enclosing class, or the {@link
 *     java.util.concurrent.Callable}'s own class when it's a named user class); always present.
