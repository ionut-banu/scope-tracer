 ...
 *
 * <ul>
 *   <li>{@code *} or {@code **} — matches any sequence of characters (including dots and any
 *       other punctuation). Adjacent asterisks are collapsed.
 *   <li>{@code ?} — matches exactly one character.
 *   <li>All other characters are matched literally; regex metacharacters are escaped.
 * </ul>
 *
 * <p>This is intentionally <b>not</b> path-segment aware: {@code com.acme.*} matches both
 * {@code com.acme.foo} and {@code com.acme.foo.bar}. Scope names and package names have no
 * canonical separator in the agent's filter API, so a single dialect is used for both. Operators
 * who want a strict prefix should write a literal pattern (e.g. {@code com.acme.checkout}).
 *
